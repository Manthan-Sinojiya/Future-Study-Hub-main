...
$stmt = $conn->prepare("SELECT id, question, option1, option2, option3, option4, audio_path, img_path FROM li_quiz");
$stmt->execute();
...
while ($row = $result->fetch_assoc()) {
    $questions[] = [
        ...
        'audio_path' => !empty($row['audio_path']) ? './uploads/' . htmlspecialchars($row['audio_path'], ENT_QUOTES, 'UTF-8') : null,
        'img_path' => !empty($row['img_path']) ? './uploads/' . htmlspecialchars($row['img_path'], ENT_QUOTES, 'UTF-8') : null
    ];
}
